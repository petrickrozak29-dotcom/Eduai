...
export const getAll = async (req: AuthRequestType, res: Response): Promise<void> => {
  try {
    ...
    sendError(res, "Gagal mengambil forum", 500, (error as Error).message);
  }
};
...
export const getById = async (req: AuthRequestType, res: Response): Promise<void> => {
  try {
    ...
    sendError(res, "Gagal mengambil detail forum", 500, (error as Error).message);
  }
};
...
export const create = async (req: AuthRequestType, res: Response): Promise<void> => {
  try {
    ...
    sendError(res, "Gagal membuat forum", 500, (error as Error).message);
  }
};

export const reply = async (req: AuthRequestType, res: Response): Promise<void> => {
  try {
    if (!req.user) {
      sendError(res, "Unauthorized", 401);
      return;
    ...
    sendError(res, "Gagal membalas forum", 500, (error as Error).message);
  }
};

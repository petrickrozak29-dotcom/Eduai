...
function buildAdaptiveMaterial(reference: string, subjectName: string) {
...
function buildQuestions(subjectName: string) {
  return [
    {
...
export const generateLearningPackage = async (req: AuthRequestType, res: Response): Promise<void> => {
  try {
    const {
      phase,
      classLevel,
    ...
    const materials = await Promise.all(
      (["TIDAK_PAHAM", "KURANG_PAHAM", "PAHAM"] as const).map((level) =>
    ...
    const quizPerBab = await Promise.all(
    ...
    sendSuccess(
      ...
      "Paket EduPath AI berhasil digenerate",
      201
    );
  } catch (error) {
    sendError(res, "Gagal generate paket EduPath AI", 500, (error as Error).message);
  }
};

export const submitPretestProfile = async (req: AuthRequestType, res: Response): Promise<void> => {
  try {
    ...
    sendError(res, "Gagal memperbarui profil belajar", 500, (error as Error).message);
  }
};

export const getLearningPath = async (req: AuthRequestType, res: Response): Promise<void> => {
  try {
    ...
    sendError(res, "Gagal mengambil learning path", 500, (error as Error).message);
  }
};

export const askAssistant = async (req: AuthRequestType, res: Response): Promise<void> => {
  try {
    ...
    sendError(res, "Gagal memproses AI Assistant", 500, (error as Error).message);
  }
};

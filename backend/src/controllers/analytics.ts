...
export const overview = async (_req: AuthRequestType, res: Response): Promise<void> => {
  try {
    const [
      totalGuru,
      totalSiswa,
    ...
    sendError(res, "Gagal mengambil analytics overview", 500, (error as Error).message);
  }
};

export const studentLearningIndex = async (req: AuthRequestType, res: Response): Promise<void> => {
  try {
    ...
    sendError(res, "Gagal mengambil Learning Index", 500, (error as Error).message);
  }
};

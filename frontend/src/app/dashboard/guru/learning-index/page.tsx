    ...
    <DashboardShell>
      <div className="space-y-5">
        <header className="rounded-lg border border-white/70 bg-white/72 p-5 shadow backdrop-blur-2xl">
          ...
          <p className="mt-4 max-w-3xl text-base leading-8 text-slate-600">Pantau Learning Index siswa per kelas secara detail.</p>
        </header>
            ...
            <div className="space-y-2">
              {classes.map((cls) => (
                <button key={cls.id} onClick={() => setSelectedClass(cls.id)}
                  className={`w-full rounded-lg border p-3 text-left transition ${selectedClass === cls.id ? "border-cyan-200 bg-cyan-50 shadow-sm" : "border-slate-200 bg-white hover:border-slate-300"}`}>
